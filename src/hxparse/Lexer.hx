...
class Lexer {
	...
	**/
	public function token<L:Lexer, T>(ruleset:Ruleset<L, T>):T {
		if (pos == input.length) {
			if (ruleset.eofFunction != null) return ruleset.eofFunction(cast this);
		...
		return ruleset.functions[lastMatch.finalId](cast this);
	}
	...
	**/
	static public function buildRuleset<L:Lexer, Token>(rules:Array<{rule:String,func:L->Token}>, name:String = "") {
		var cases = [];
		var functions = [];
		...
		return new Ruleset<L, Token>(new LexEngine(cases).firstState(), functions, eofFunction, name);
	}
}