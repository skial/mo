...
class LexerGraph<L:Lexer, T> {
	
	static public function printRuleset<L:Lexer, T>(ruleset:Ruleset<L, T>):String {
		var lexerGraph = new LexerGraph(ruleset);
		return lexerGraph.graph.getDotCode();
	}
	
	var graph:Graph;
	var ruleset:Ruleset<L, T>;
	var map:Map<State, Node>;
	
	function new(ruleset:Ruleset<L, T>) {
